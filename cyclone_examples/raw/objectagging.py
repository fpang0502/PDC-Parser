...
def main():
	if sys.argv[1] == "cyclone":
		cyclone = Warnings()
		extract_cyclone()
	elif sys.argv[1] == "volcano":
		volcano = Warnings()
		extract_volcano()
	elif sys.argv[1] == "tsunami":
		tsunami = Warnings()
		extract_tsunami()
	else:
		print("Please type a valid warning type.")
		return
	infile=sys.argv[1]
...
class Warnings:
	def print4tab(tag, text, outfile):
		#Print to outfile with 4 tabs
		outfile.write("\t\t\t\t<" + tag + ">" + text + "</" + tag + ">\n")

	def print5tab(tag, text, outfile):
		outfile.write("\t\t\t\t\t<" + tag + ">" + text + "</" + tag + ">\n")

	def print6tab(tag, text, outfile):
		outfile.write("\t\t\t\t\t\t<" + tag + ">" + text + "</" + tag + ">\n")
