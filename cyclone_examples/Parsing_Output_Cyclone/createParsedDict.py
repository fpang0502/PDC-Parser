...
def analyzeList(myList):
	"""
	creates a list of dictionaries and stores the data of the index, word, head
	(which word points this the current word) and the typed dependency'
	"""
	master_list =[]

	for x in myList:
		dict_list = analyzeString(x)
...
def analyzeString(string):
	"""
	creates a dictionary and stores the data of the index, word, head
	(which word points this the current word) and the typed dependency'
	"""
	dict_list = []
	mySent = getTypeD(string)
