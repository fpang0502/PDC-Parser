...
tag_space = {
	'xml open':0, 'hazards open':1, 'hazards close':1, 'tsunami open':2,
	'tsunami close':2, 'incident open':3, 'incident close':3, 'wmoID':4,
	'station':4, 'ddhhmm':4, 'awips':4, 'UGC':4, 'stateID':5, 'UGCFormat':5,
	'code':5, 'purgeTime':5, 'bulletinNumber':4, 'issuer':4, 'issueTo':4,
	'subject':4, 'brief':4, 'overview':4, 'time':4, 'declaration':4,
	'event open':4, 'event close':4, 'magnitude open':5, 'magnitude close':5,
	'value':6, 'scale':6, 'eventTime':5, 'latitude':5, 'longitude':5,
	'depth':5, 'location':5, 'evaluation':4, 'additionalInfo':4, 'UGC open':4,
	'UGC close':4, 'e_time':5, 'earthquake':5
}

opener = [
	'xml open', 'hazards open', 'tsunami open', 'incident open'
]
header_info = [
	'wmoID', 'station','ddhhmm','awips','UGC open','stateID',
	'UGCFormat','code','purgeTime','UGC close'
]
specifics = [
	'bulletinNumber','issuer','time','issueTo','subject','brief','overview'
]
event = [
	'event open','earthquake','e_time','latitude','longitude',
	'location','magnitude open','value', 'magnitude close', 'scale',
	'event close'
]
ev_ai = [
...
closer = [
	'incident close','tsunami close','hazards close'
]
			...
			if tag_name == 'bulletinNumber':
				...
				for k,v in curr.items():
					...
					if ((k=='word' and v=='statement') or (k=='word' and v=='message')) and curr['deprel'] == 'nsubj':
						print("found the match for k as word and v as statement or message and deprel as nsubj")
						headNum = curr['head']
						print(headNum)
						check = s[headNum-1]
						print(check)
				...
				for k,v in curr.items():
					...
					if k=='word' and v =='preliminary' and curr['deprel'] == 'amod':
						print("found the match for k as word and v as preliminary and deprel as amod")
						headNum = curr['head']
						print(headNum)
						check = s[headNum-1]
						print(check)
